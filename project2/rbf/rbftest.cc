...
int RBFTest_jay(PagedFileManager *pfm)
{
    // Functions Tested:
    // Create File
    // Open File
    ...
    // Write Page again
    // Destroy File
    cout << "****In RBF Test Case Jay****" << endl;

    RC rc;
    string fileName = "test_jay";
    ...
    //Append pages
    void *data0 = malloc(PAGE_SIZE);
    for(unsigned i = 0; i < PAGE_SIZE; i++)
    {
        *((char *)data0+i) = i % 94 + 32;
    }
    FileHandle fileHandle0;
    rc = pfm->openFile(fileName.c_str(), fileHandle0);
    fileHandle0.appendPage(data0);
    fileHandle0.appendPage(data0);
    rc = pfm->closeFile(fileHandle0);
    assert(rc == success);

    // Open the file "test_jay"
    FileHandle fileHandle;
    rc = pfm->openFile(fileName.c_str(), fileHandle);
    assert(rc == success);
    ...
    for(unsigned i = 0; i < PAGE_SIZE; i++)
    {
        *((char *)data2+i) = i % 10 + 32;
    }
    rc = fileHandle2.writePage(8, data2);
    assert(rc == fail);
    ...
    {
        cout << "File " << fileName << " has been destroyed." << endl;
        cout << "Test Case Jay Passed!" << endl << endl;
        return 0;
    }
    else
    {
        cout << "Failed to destroy file!" << endl;
        cout << "Test Case Jay Failed!" << endl << endl;
        return -1;
    }
}
