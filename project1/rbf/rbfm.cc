...
RC RecordBasedFileManager::insertRecord(FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const void *data, RID &rid) {
    ...
    int offset = m_pageIndexTracker->GetNextOffset();

    //Space remaining = size of page - size of current data - size of index - size of new index (from the data that's about to be inserted) - length of record bytes
    int spaceRemaining = PAGE_SIZE - offset - m_pageIndexTracker->GetIndexSize() - sizeof(unsigned int) - sizeof(unsigned int);
    ...
    //If not enough space on the page, append a page and write data to new page
    else
    {
